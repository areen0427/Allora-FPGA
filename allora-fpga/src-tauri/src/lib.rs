...
fn detect_connected_board(
    ...
    let (programmer_detected, programmer_details) = if tool_path.exists() {
        (
            true,
            format!("{} found at {}", command, tool_path.display()),
        )
    } else {
        ...
        if let Ok(output) = output {
            ...
            for line in text.lines() {
                ...
                if trimmed.contains("Product ID:") {
                    current_product_id = trimmed.split(':').nth(1).unwrap_or("").trim().to_string();
                }
                if trimmed.contains("Vendor ID:") {
                    current_vendor = trimmed.split(':').nth(1).unwrap_or("").trim().to_string();
                }
                if trimmed.contains("Product:") || trimmed.contains("Name:") {
                    let name = trimmed.split(':').nth(1).unwrap_or("").trim().to_string();
                    if !name.is_empty() {
                        current_device = name;
                    }
                }
                if trimmed.contains("Manufacturer:") {
                    let mfr = trimmed.split(':').nth(1).unwrap_or("").trim().to_string();
                ...
                if trimmed.is_empty() && !current_device.is_empty() {
                    let lower = format!(
                        "{} {} {}",
                        current_device, current_vendor, current_product_id
                    )
                    .to_lowercase();
                    if lower.contains("ftdi")
                        || lower.contains("lattice")
                        || lower.contains("digilent")
                        || lower.contains("sipeed")
                        || lower.contains("tinyfpga")
                        || lower.contains("fomu")
                        || lower.contains("1bit")
                        || lower.contains("1209")
                        || lower.contains("0403")
                        || lower.contains("6010")
                        || lower.contains("cmsis")
                        || lower.contains("jtag")
                        || lower.contains("ft2232")
                        || lower.contains("ft232")
                        || lower.contains("ftdi")
                    {
                        let possible_boards = identify_possible_boards(
                            &current_device,
                            &current_vendor,
                            &current_product_id,
                        );
...
fn rename_project_file(request: RenameProjectFileRequest) -> Result<(), ErrorPayload> {
    ...
    if let Some(parent) = to_path.parent() {
        fs::create_dir_all(parent).map_err(|err| {
            error(&format!(
                "Unable to create the destination directory: {err}"
            ))
        })?;
    }
...
fn detect_programmer(
    ...
    let output = Command::new(&tool_path)
        .arg("--help")
        .output()
        .or_else(|_| Command::new(&tool_path).arg("-h").output())
        .or_else(|_| Command::new(&tool_path).arg("--version").output());
    ...
    match output {
        Ok(result) => {
            ...
            let version_line = combined
                .lines()
                .find(|line| {
                    line.to_lowercase().contains("version") || line.to_lowercase().contains(command)
                })
            ...
            if result.status.success() || !stdout.trim().is_empty() {
                ...
                Ok(DetectProgrammerResponse {
                    ...
                    tool_path: None,
                    message: format!(
                        "{command} was not found. Install it to enable FPGA programming."
                    ),
                })
            }
        }
...
async fn program_fpga(
    ...
    if status.success() {
        ...
        send_log(
            &on_log,
            &mut logs,
            "[programming] ✓ Programming completed successfully.",
        );
        Ok(ProgramFpgaResponse {
            success: true,
            logs,
            message: format!(
                "Successfully programmed {} with {}.",
                request.board_name, request.bitstream_path
            ),
        })
    } else {
        ...
        send_log(
            &on_log,
            &mut logs,
            format!("[programming] ✗ {error_details}"),
        );
        Ok(ProgramFpgaResponse {
            success: false,
            logs,
...
fn tool_is_available(path: &Path) -> bool {
    if path.components().count() > 1 {
        return path.exists();
    }
...
fn launch_codex_login_terminal(codex_path: &Path) -> Result<(), ErrorPayload> {
    #[cfg(target_os = "macos")]
    {
        let command = format!(
            "do script \"{} login --device-auth\"",
            shell_escape(codex_path.display().to_string())
        );
        Command::new("osascript")
            ...
            .output()
            .map_err(|err| error(&format!("Unable to open Terminal for Codex sign-in: {err}")))?;
        return Ok(());
    }
    ...
    #[cfg(target_os = "windows")]
    {
      ...
      .args([
        ...
        "-Command",
        &format!(
          "Start-Process powershell -ArgumentList '-NoExit','-Command','\"{}\" login --device-auth'",
          codex_path.display()
        ),
      ])
      .output()
      .map_err(|err| error(&format!("Unable to open PowerShell for Codex sign-in: {err}")))?;
        return Ok(());
    }
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        return Err(error(
      "Unable to open a terminal for Codex sign-in. Run `codex login --device-auth` in a terminal.",
    ));
    }
}
...
#[derive(Debug, Serialize)]
#[serde(rename_all = "camelCase")]
struct CodexPromptResponse {
    response: String,
    workspace_changed: bool,
}

#[tauri::command]
fn codex_auth_status() -> Result<CodexAuthStatusResponse, ErrorPayload> {
    let codex_path = resolve_tool_path("codex");
    if !tool_is_available(&codex_path) {
        return Ok(CodexAuthStatusResponse {
      available: false,
      signed_in: false,
      account_label: None,
      details: "Official Codex CLI was not found on PATH. Install Codex and sign in with your own ChatGPT/OpenAI account.".to_string(),
      codex_path: None,
    });
    }
    ...
    let output = Command::new(&codex_path)
        .args(["login", "status"])
        .output()
        .map_err(|err| error(&format!("Unable to check Codex login status: {err}")))?;

    let stdout = String::from_utf8_lossy(&output.stdout).trim().to_string();
    let stderr = String::from_utf8_lossy(&output.stderr).trim().to_string();
    let details = if stdout.is_empty() { stderr } else { stdout };
    let lower_details = details.to_lowercase();
    let signed_in = output.status.success()
        && (lower_details.contains("logged in") || lower_details.contains("signed in"));

    Ok(CodexAuthStatusResponse {
        available: true,
        signed_in,
        account_label: if signed_in {
            Some(details.clone())
        } else {
            None
        },
        details: if details.is_empty() {
            "Codex is installed, but sign-in status could not be determined.".to_string()
        } else {
            details
        },
...
#[tauri::command]
fn start_codex_login() -> Result<CodexAuthStatusResponse, ErrorPayload> {
    let codex_path = resolve_tool_path("codex");
    if !tool_is_available(&codex_path) {
        return Ok(CodexAuthStatusResponse {
      available: false,
      signed_in: false,
      account_label: None,
      details: "Official Codex CLI was not found on PATH. Install Codex first, then sign in with your own ChatGPT/OpenAI account.".to_string(),
      codex_path: None,
    });
    }

    launch_codex_login_terminal(&codex_path)?;

    Ok(CodexAuthStatusResponse {
    available: true,
    signed_in: false,
    account_label: None,
    details: "A Codex sign-in window was opened. Complete the OpenAI/ChatGPT login there, then reopen or refresh Allora Codex.".to_string(),
    codex_path: Some(codex_path.display().to_string()),
  })
}

#[tauri::command]
fn codex_logout() -> Result<CodexAuthStatusResponse, ErrorPayload> {
    let codex_path = resolve_tool_path("codex");
    if !tool_is_available(&codex_path) {
        return Ok(CodexAuthStatusResponse {
            available: false,
            signed_in: false,
            account_label: None,
            details: "Official Codex CLI was not found on PATH.".to_string(),
            codex_path: None,
        });
    }
    ...
    let output = Command::new(&codex_path)
        .arg("logout")
        .output()
        .map_err(|err| error(&format!("Unable to sign out of Codex: {err}")))?;

    if !output.status.success() {
        return Err(error(&format_command_error(
            "codex logout",
            &output.stdout,
            &output.stderr,
        )));
    ...
    Ok(CodexAuthStatusResponse {
        ...
        signed_in: false,
        account_label: None,
        details: "Signed out of Codex on this machine.".to_string(),
        codex_path: Some(codex_path.display().to_string()),
    })
}

#[tauri::command]
fn send_codex_prompt(request: CodexPromptRequest) -> Result<CodexPromptResponse, ErrorPayload> {
    let prompt = request.prompt.trim();
    if prompt.is_empty() {
        return Err(error("Enter a prompt for Allora Codex."));
    }

    let codex_path = resolve_tool_path("codex");
    if !tool_is_available(&codex_path) {
        return Err(error("Official Codex CLI was not found on PATH."));
    }

    let mut command = Command::new(&codex_path);
    let agent_mode = request.mode == "agent";
    let sandbox_mode = if agent_mode {
        "workspace-write"
    } else {
        "read-only"
    };
    ...
    {
        command.args(["--cd", project_path]);
    }

    let output = command
        .arg(prompt)
        .output()
        .map_err(|err| error(&format!("Unable to run Codex: {err}")))?;
    ...
    if response.is_empty() {
        return Err(error("Codex finished without returning a message."));
    }

    Ok(CodexPromptResponse {
        response,
        workspace_changed: agent_mode,
    })
}
...
async fn generate_synthesis_diagram(
    ...
    if request.files.iter().any(|file| is_vhdl_file(&file.name)) {
        return Err(error(
...
async fn generate_bitstream(
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("[bitstream] Top module: {top_module}"),
    );
    ...
    let (yosys_status, yosys_stdout, yosys_stderr) = run_command_streaming(
        &tool_command("yosys"),
        &yosys_args,
        Some(&temp_dir),
        &on_log,
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("{pnr_command} {}", pnr_args.join(" ")),
    );
    let (pnr_status, pnr_stdout, pnr_stderr) = run_command_streaming(
        &tool_command(&pnr_command),
        &pnr_args,
        Some(&temp_dir),
        &on_log,
    )
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("{pack_command} {}", pack_args.join(" ")),
    );
    let (pack_status, pack_stdout, pack_stderr) = run_command_streaming(
        &tool_command(&pack_command),
        &pack_args,
        Some(&temp_dir),
        &on_log,
    )
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("[output] Generated {} bytes", bytes.len()),
    );
...
async fn simulate_testbench(
    ...
    fs::write(&testbench_path, &request.testbench_file.content).map_err(|err| {
        error(&format!(
            "Unable to write {}: {err}",
            request.testbench_file.name
        ))
    })?;
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("iverilog {}", iverilog_args.join(" ")),
    );
    let (compile_status, compile_stdout, compile_stderr) = run_command_streaming(
    ...
    if !compile_status.success() {
        let _ = fs::remove_dir_all(&temp_dir);
        let error_details =
            format_command_error("Icarus Verilog", &compile_stdout, &compile_stderr);
        return Err(error(&error_details));
    }
    ...
    let project_waveform_path = if let Some(project_path) = &request.project_path {
        ...
        fs::create_dir_all(&build_dir).map_err(|err| {
            error(&format!(
                "Unable to create simulation output directory: {err}"
            ))
        })?;
    ...
    send_log(
        &on_log,
        &mut logs,
        format!("[output] Waveform bytes: {}", vcd.len()),
    );
...
fn build_graph(
        ...
        for (name, port_value) in ports {
            ...
            for bit in bits {
                if direction == "input" || direction == "inout" {
                    drivers.entry(bit.clone()).or_default().push(DriverRef {
                        node_id: node_id.clone(),
                    });
                }
                if direction == "output" || direction == "inout" {
                    consumers.entry(bit.clone()).or_default().push(ConsumerRef {
                        node_id: node_id.clone(),
                    });
                }
            }
    ...
    if let Some(cells) = module.get("cells").and_then(Value::as_object) {
        for (name, cell_value) in cells {
            ...
            let port_directions = cell_value.get("port_directions").and_then(Value::as_object);
...
fn display_bit_label(bit: &str) -> String {
    bit.strip_prefix("bit:")
        .map(|value| format!("net {value}"))
        .or_else(|| {
            bit.strip_prefix("const:")
...
fn collect_workspace_files(
    ...
    for entry in entries {
        let entry =
            entry.map_err(|err| error(&format!("Unable to read workspace entry: {err}")))?;
        let path = entry.path();
        let file_type = entry
            .file_type()
...
fn build_yosys_script(request: &GenerateSynthesisDiagramRequest, json_path: &Path) -> String {
...
fn build_bitstream_yosys_script(request: &GenerateBitstreamRequest, json_path: &Path) -> String {
    let mut lines = Vec::<String>::new();
...
fn find_first_verilog_module(content: &str) -> Option<String> {
    let mut tokens = content.split(|character: char| {
...
fn create_project_workspace(
    ...
    for file_spec in &request.files {
        ...
        fs::write(&absolute, &file_spec.content).map_err(|err| {
            error(&format!(
                "Unable to write {}: {err}",
...
fn parse_iverilog_diagnostics(output: &str) -> Vec<LintDiagnostic> {
    ...
    for line in output.lines() {
        ...
        let Some(raw_message) = parts.next() else {
            continue;
        };
        ...
        let file_name = raw_path.trim().trim_start_matches("src/").to_string();
