...
function button(name: string, pin: string, activeLow = false): BoardPin {
  return { name, signal: name, pin, type: "button", group: "User Buttons", activeLow, verified: true };
}
...
function spi(name: string, signal: string, pin: string, activeLow = false): BoardPin {
  return { name, signal, pin, type: "spi", group: "SPI Flash", activeLow, verified: true };
}

function gpio(name: string, signal: string, pin: string, group = "GPIO"): BoardPin {
...
function makeIce40Board({
  ...
  return {
    ...
    clocks: [{ name: "clk12", pin: clockPin, frequency: 12000000, verified: true }],
    pins,
    leds,
    buttons,
    notes,
...
function makeEcp5Board({
  ...
  return {
    ...
    clocks: [{ name: clockName, pin: clockPin, frequency: clockFrequency, verified: true }],
    pins,
    leds,
    buttons,
    notes,
...
function makeBitsyRevision(revision: "v0" | "v1"): BoardDefinition {
  const leds = revision === "v0"
    ? [led("user_ledr_n", "11", "rgb_red"), led("user_ledg_n", "37", "rgb_green")]
    : [led("user_ledr_n", "25", "rgb_red"), led("user_ledg_n", "6", "rgb_green"), led("rgb_led_b", "41", "rgb_blue")];
...
const ICEPI_ZERO_BUTTONS = [button("user_btn_0", "C4"), button("user_btn_1", "C5")];
function makeIcepiZero(device: "25F" | "45F"): BoardDefinition {
  return makeEcp5Board({
    ...
    notes: "Pin mapping based on the LiteX iCEpi Zero platform. SDRAM, USB, GPIO header, and GPDI buses are summarized.",
  });
}
...
export const latticeEcp5Evn = makeEcp5Board({
  ...
  program: "openocd",
  leds: [0, 1, 2, 3, 4, 5, 6, 7].map((index) => led(`user_led_${index}`, ["A13", "A12", "B19", "A18", "B18", "C17", "A17", "B17"][index], `led${index}`)),
  buttons: [button("button_1", "P4")],
  pins: [
    ...[0, 1, 2, 3, 4, 5, 6, 7].map((index) => led(`user_led_${index}`, ["A13", "A12", "B19", "A18", "B18", "C17", "A17", "B17"][index], `led${index}`)),
    button("button_1", "P4"),
  ...
  notes: "Pin mapping based on the LiteX Lattice ECP5 EVN platform. SERDES and Raspberry Pi connector signals are summarized.",
});
...
export const latticeVersaEcp5 = makeEcp5Board({
  ...
  program: "openocd",
  leds: [0, 1, 2, 3, 4, 5, 6, 7].map((index) => led(`user_led_${index}`, ["E16", "D17", "D18", "E18", "F17", "F18", "E17", "F16"][index], `led${index}`)),
  buttons: [button("rst_n", "T1", true)],
  pins: [
    ...[0, 1, 2, 3, 4, 5, 6, 7].map((index) => led(`user_led_${index}`, ["E16", "D17", "D18", "E18", "F17", "F18", "E17", "F16"][index], `led${index}`)),
  ...
  notes: "Pin mapping based on the LiteX Lattice Versa ECP5 platform. DDR3, PCIe, Ethernet, and SMA buses are summarized.",
});
...
export const trellisBoard = makeEcp5Board({
  ...
  notes: "Pin mapping based on the LiteX TrellisBoard platform. DDR3, PCIe, Ethernet, DVI, and PMOD buses are summarized.",
});
...
function makeKosagiNetV2(variant: "a7-35" | "a7-100"): BoardDefinition {
  ...
  return makeVivadoBoard({
    ...
    pins: [led("user_led", "M21"), uart("serial_tx", "uart_tx", "M19"), uart("serial_rx", "uart_rx", "M20")],
    notes: "Pin mapping based on the LiteX Kosagi NetV2 platform. Vivado runner is needed for synthesis and bitstream generation.",
  });
}

function makeSqrlAcorn(variant: "cle-101" | "cle-215" | "cle-215+"): BoardDefinition {
  const device = variant === "cle-101" ? "XC7A100T" : "XC7A200T";
  const fpgaId = variant === "cle-101"
    ? "xc7a100t-fgg484-2"
    : variant === "cle-215"
      ? "xc7a200t-fbg484-2"
      : "xc7a200t-fbg484-3";
  return makeVivadoBoard({
    ...
    notes: "Pin mapping based on the LiteX SQRL Acorn platform. Vivado runner is needed for synthesis and bitstream generation.",
  });
}
...
export const sqrlFk33 = makeVivadoBoard({
  ...
  leds: [],
  pins: [],
  notes: "Pin mapping based on the LiteX SQRL FK33 platform. Vivado runner is needed for synthesis and bitstream generation.",
});

export const icebreakerBitsyBoards = [makeBitsyRevision("v0"), makeBitsyRevision("v1")];
export const icepiZeroBoards = [makeIcepiZero("25F"), makeIcepiZero("45F")];
export const kosagiNetV2Boards = [makeKosagiNetV2("a7-35"), makeKosagiNetV2("a7-100")];
export const sqrlAcornBoards = [makeSqrlAcorn("cle-101"), makeSqrlAcorn("cle-215"), makeSqrlAcorn("cle-215+")];
